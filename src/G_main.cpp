...
void Handle_Gens_Messages()
{
	...
	while (PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE))
	{
		...
		/*if (!out)
		{
			out=fopen("qqwweezz.txt","w+");
		}
		GetWindowText(msg.hwnd,WindowName,20);
		fprintf(out,"%-19s %s %08x %08x\n",WindowName,GetMsgName(msg.message),msg.wParam,msg.lParam);*/
...
dialogAgain: //Nitsuja added this
				...
				case ID_FILES_GAMEGENIE:
					...
					{
						DialogsOpen++;
						GameGenieHWnd = CreateDialog(ghInstance, MAKEINTRESOURCE(IDD_GAMEGENIE), hWnd, (DLGPROC) GGenieProc);
					}
					else
						SetForegroundWindow(GameGenieHWnd);
					return 0;
...
HMENU Build_Main_Menu(void)
{
	...
	MENU_L(GraphicsLayers, i++, Flags | MF_POPUP, (UINT)GraphicsLayersX, "32X", "", "32&X");
	...
	MENU_L(GraphicsLayersX, i, MF_BYPOSITION | (_32X_Plane_Low_On ? MF_CHECKED : MF_UNCHECKED), ID_GRAPHICS_LAYER32X_LOW, "32X Plane Low", "", "32X Plane &Low");
	...
	MENU_L(GraphicsLayersX, i, MF_BYPOSITION | (_32X_Plane_High_On ? MF_CHECKED : MF_UNCHECKED), ID_GRAPHICS_LAYER32X_HIGH, "32X Plane High", "", "32X Plane &High");
	...
	MENU_L(GraphicsLayersX, i, MF_BYPOSITION | (Swap_32X_Plane_Priority ? MF_CHECKED : MF_UNCHECKED), ID_GRAPHICS_LAYERSWAP32X, "Swap 32X Plane Layers", "", "&Swap 32X Plane Layers");
	...
	MENU_L(GraphicsLayersX, i, MF_BYPOSITION | (_32X_Plane_On ? MF_CHECKED : MF_UNCHECKED), ID_GRAPHICS_TOGGLE32X, "Enable", "", "&Enable");
	...
	MENU_L(GraphicsLayersS, i++, MF_BYPOSITION | (Sprite_Boxing ? MF_CHECKED : MF_UNCHECKED), ID_GRAPHICS_SPRITEBOXING, "Sprites Boxing", "", "Sprites Boxing");
...
//	MENU_L(Sound, i++, Flags | (PSG_Improv ? MF_CHECKED : MF_UNCHECKED), ID_SOUND_PSGIMPROV, "PSG High Quality", "", "PSG High &Quality");
	...
	MENU_L(Tools_Trace, i++, Flags | (trace_indent ? MF_CHECKED : MF_UNCHECKED), ID_CHANGE_TRACE_INDENT, "Trace Indentation", "", "Trace Indentation");
...
RECT GGRect;
WNDPROC lastGGListProc;

LRESULT CALLBACK GGListProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	if (uMsg==WM_RBUTTONUP)
	{
		LRESULT res=SendMessage(hWnd,LB_ITEMFROMPOINT,0,lParam);
		if (!HIWORD(res))
			SendMessage(hWnd,LB_DELETESTRING,(WPARAM)LOWORD(res),0);
	}
	return CallWindowProc(lastGGListProc,hWnd,uMsg,wParam,lParam);
}
...
LRESULT CALLBACK GGenieProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{
		case WM_INITDIALOG:
			GetClientRect(hDlg,&GGRect);
			lastGGListProc=(WNDPROC)SetWindowLong(GetDlgItem(hDlg,IDC_LIST1),GWL_WNDPROC,(LONG)GGListProc);
			Build_Language_String();
		...
		case WM_COMMAND:
			...
			{
				case ID_GGADD:
					if (GetDlgItemText(hDlg, IDC_EDIT1, Str_Tmp, 1023))
					{
						if (strchr(Str_Tmp,',') && !strchr(strchr(Str_Tmp,',')+1,','))
						{
							int i,j=0;
							for (i=0;Str_Tmp[i];++i)
								if ((Str_Tmp[i]>='a'&&Str_Tmp[i]<='f')
									||(Str_Tmp[i]>='A'&&Str_Tmp[i]<='F')
									||(Str_Tmp[i]>='0'&&Str_Tmp[i]<='9')
									||Str_Tmp[i]==',')
								Str_Tmp[j++]=Str_Tmp[i];
								else if (Str_Tmp[i]!=' '&&Str_Tmp[i]!='\t')
									return true;
							Str_Tmp[j++]=' ';
							Str_Tmp[j]=0;
							GetDlgItemText(hDlg, IDC_EDIT2, (char *) (Str_Tmp + j), 1024 - j);
						...
						else if ((strlen(Str_Tmp) == 9) || (strlen(Str_Tmp) == 11))
						{						
							strupr(Str_Tmp);
							while (strlen(Str_Tmp) < 20) strcat(Str_Tmp, " ");
		...
		case WM_CLOSE:
			SetWindowLong(GetDlgItem(hDlg,IDC_LIST1),GWL_WNDPROC,(LONG)lastGGListProc);
			GameGenieHWnd = NULL;
			DialogsOpen--;
			EndDialog(hDlg, true);
			return true;
		...
		case WM_SIZE:
			if (wParam != SIZE_MINIMIZED)
			{
				...
				RECT rc;
				POINT P;
				GetWindowRect(GetDlgItem(hDlg,IDC_LIST1),&rc);
				SetWindowPos(GetDlgItem(hDlg,IDC_LIST1),HWND_TOP,0,0,
					rc.right-rc.left+x-GGRect.right,
					rc.bottom-rc.top+y-GGRect.bottom,SWP_NOMOVE|SWP_NOZORDER);
#define MOVERIGHT(a) GetWindowRect(GetDlgItem(hDlg,a),&rc); \
				P.x=rc.left; \
				P.y=rc.top; \
				ScreenToClient(hDlg, &P); \
				SetWindowPos(GetDlgItem(hDlg,a),HWND_TOP, \
					P.x+x-GGRect.right, \
					P.y,0,0,SWP_NOSIZE|SWP_NOZORDER);
				MOVERIGHT(ID_GGADD);
				MOVERIGHT(ID_GGREMOVE);
				MOVERIGHT(ID_GGDEACTIVATE);
				MOVERIGHT(ID_OK);
				MOVERIGHT(ID_CANCEL);
				MOVERIGHT(ID_GGAPPLY);
#undef MOVERIGHT

#define SIZERIGHT(a) GetWindowRect(GetDlgItem(hDlg,a),&rc); \
				SetWindowPos(GetDlgItem(hDlg,a),HWND_TOP,0,0, \
					rc.right-rc.left+x-GGRect.right, \
					rc.bottom-rc.top,SWP_NOMOVE|SWP_NOZORDER);
				SIZERIGHT(IDC_EDIT1);
				SIZERIGHT(IDC_EDIT2);
				SIZERIGHT(IDC_INFO_GG);
#undef SIZERIGHT
				GGRect.right=x;
				GGRect.bottom=y;
			}
